...
class TestLLMTracer:
    ...
    async def test_trace_generate_succeeds(self):
        async with self.tracer.trace_generate(model="test-model", intent="question"):
            pass  # simulate LLM call

    @pytest.mark.asyncio
    async def test_trace_generate_with_exception(self):
        with pytest.raises(ValueError):
            async with self.tracer.trace_generate():
                raise ValueError("test error")

    @pytest.mark.asyncio
    async def test_trace_generate_with_slots(self):
        async with self.tracer.trace_generate(slot_count=3):
            pass
