...
class Settings(BaseSettings):
    ...
    DIALOGUE_CHECKPOINTER_DB_URL: str | None = Field(
        default=None,
        description="Postgres URL for the dialogue checkpointer (defaults to DATABASE_URL)",
    )
    ...
    RATE_LIMIT_ENABLED: bool = Field(
        default=True, description="Enable global rate limiting middleware"
    )
    RATE_LIMIT_REQUESTS_PER_MINUTE: int = Field(
        default=120, description="Requests allowed per client IP per window"
    )
    ...
    RATE_LIMIT_REDIS_URL: str | None = Field(
        default=None, description="Dedicated Redis URL for rate limiting; defaults to REDIS_URL"
    )
    ...
    EMBEDDING_CACHE_TTL: int = Field(
        default=604800, description="Embedding cache TTL in seconds (7 days)"
    )
    ...
    MEMORY_TYPE: str = Field(
        default="optimized",
        description="Memory strategy (sliding_window, summarization, hybrid, optimized)",
    )
    ...
    MEMORY_RELEVANCE_THRESHOLD: float = Field(
        default=0.5, description="Minimum similarity for relevance filtering"
    )
    ...
    GRAPH_RAG_ENABLED: bool = Field(default=False, description="Enable GraphRAG features")
